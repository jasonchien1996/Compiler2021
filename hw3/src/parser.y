...
decl_list:
decl_list decl  { $$ = makeSibling($1, $2); }
| decl          { $$ = $1; }
;
...
| TYPEDEF VOID id_list MK_SEMICOLON {
                                        $$ = makeDeclNode(TYPE_DECL);
                                        makeChild($$, makeSibling(makeIDNode("void", NORMAL_ID), $3));
                                    }
;
...
| ID init_id_list MK_SEMICOLON  {
                                    $$ = makeDeclNode(VARIABLE_DECL);
                                    makeChild($$, makeSibling(makeIDNode($1, NORMAL_ID), $2));
                                }
;
...
| id_list MK_COMMA ID           { $$ = makeSibling($1, makeIDNode($3, NORMAL_ID)); }
| id_list MK_COMMA ID dim_decl  { $$ = makeSibling($1, makeChild(makeIDNode($3, ARRAY_ID), $4)); }
| ID dim_decl                   { $$ = makeChild(makeIDNode($1, ARRAY_ID), $2); }
;
...
init_id_list: 
init_id                         {   $$ = $1;   }
| init_id_list MK_COMMA init_id {   $$ = makeSibling($1, $3);  }
;
...
stmt_list:
stmt_list stmt  {   $$ = makeSibling($1, $2); }
| stmt          {   $$ = $1;    }
;

stmt:
MK_LBRACE block MK_RBRACE                                   {   $$ = $2;   }
...
| IF MK_LPAREN nonempty_relop_expr_list MK_RPAREN stmt      {   $$ = makeStmtNode(IF_STMT);
...
| RETURN MK_SEMICOLON                                       {   $$ = makeStmtNode(RETURN_STMT);   }
...
nonempty_assign_expr_list:
nonempty_assign_expr_list MK_COMMA assign_expr  { $$ = makeSibling($1, $3); }
| assign_expr                                   { $$ = $1; }
;
...
relop_factor:
expr                { $$ = $1; }
| expr rel_op expr  { $$ = makeFamily($2, 2, $1, $3); }
;

rel_op:
OP_EQ   { $$ = makeExprNode(BINARY_OPERATION, BINARY_OP_EQ); }
...
nonempty_relop_expr_list: 
nonempty_relop_expr_list MK_COMMA relop_expr    { $$ = makeSibling($1, $3); }
| relop_expr                                    { $$ = $1; }
;

expr:
expr add_op term    { $$ = makeFamily($2, 2, $1, $3); }
| term              { $$ = $1; }
;
...
term:
term mul_op factor  { $$ = makeFamily($2, 2, $1, $3); }
| factor            { $$ = $1; }
;

mul_op:
OP_TIMES    { $$ = makeExprNode(BINARY_OPERATION, BINARY_OP_MUL); }
| OP_DIVIDE { $$ = makeExprNode(BINARY_OPERATION, BINARY_OP_DIV); }
;

factor:
MK_LPAREN relop_expr MK_RPAREN          { $$ = $2; }
...
| ID MK_LPAREN relop_expr_list MK_RPAREN{   $$ = makeFamily(makeStmtNode(FUNCTION_CALL_STMT), 2, makeIDNode($1, NORMAL_ID), $3); }
...
dim_list:
dim_list MK_LB expr MK_RB   { $$ = makeSibling($1, $3); }
| MK_LB expr MK_RB          { $$ = $2; }
;
